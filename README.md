...
### Standalone tools (no placeholders)
...
#### [claude-to-claude](skills/claude-to-claude/)

Hand off **ownership** of a Claude Code project to another person so their Claude arrives already briefed and actively onboards them through the transition -- an ownership transfer of code *and* understanding, not a settings copy. Two modes: **sender** (`/claude-to-claude handoff`) mines the repo, runs a mandatory exit-interview, sanitizes secrets, and assembles a provenance-tagged `.handoff/` bundle committed into the repo; **recipient** (`/claude-to-claude onboard`) verifies the bundle, guides environment reconstruction with smoke tests, gives a calibrated tour of the architecture and decisions, proposes a safe first task, and *graduates* the transition (archiving the scaffolding) once the new owner has settled in. Secrets travel by name only; the incoming bundle is treated as untrusted input (no hooks auto-run). Bundles four Python scripts (mine / secret-scan / assemble / verify) and five reference specs. Depends only on Python 3 and git. See the [skill README](skills/claude-to-claude/README.md) for the full workflow.
